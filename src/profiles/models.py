...
class ProfileManager(models.Manager):
    def get_all_profiles_to_invite(self, sender):
        ...
        # check two cases
        # we are the sender or we are the receiver
        ...
        for related in related_relationships:
            if related.status == "accepted":
                accepted.add(related.receiver)
                accepted.add(related.sender)
        print("related_relationships after accepted: ", related_relationships)
...
class Profile(models.Model):
    ...
    avatar = models.ImageField(
        default="static_root/img/avatar.png", upload_to="avatars/"
    )
...
class RelationshipManager(models.Manager):
    def invitations_received(self, receiver):
        sender = Relationship.objects.filter(receiver=receiver, status="send")
        return sender
